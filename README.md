...
| Magic speed, health & jump | **very common** | Three more genes just like magic body size, one each for **how fast** a horse runs, **how many hearts** it has, and **how high it jumps** - about a tenth either way per copy, that horse's own number, passed to its foals. Most wild horses carry a copy of all four, so a wild-caught horse has a small wobble on every axis; the horses that are genuinely fast, tough or springy are bred, two good copies at a time. One of each kind cancels out. |
...
| Cutie mark | **hidden** | A little **emblem of one to three item icons** stamped on **both flanks**, drawn on top of everything else - white patterns included. Which items, how many, and whether they sit in a row or a triangle are all that horse's own and **pass to its foals**, so a mark you like breeds true. Any flat item in the game is fair game, modded ones included. Both parents have to pass the gene on. |
