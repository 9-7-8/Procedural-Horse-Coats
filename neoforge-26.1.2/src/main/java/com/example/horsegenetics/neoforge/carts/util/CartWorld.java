...
public class CartWorld extends SavedData {

    public static final Codec<CartWorld> CODEC = RecordCodecBuilder.create(instance ->
            instance.group(EntityWithId.CODEC
                    ...
                    .optionalFieldOf("drawnList", List.of()).forGetter((world) ->
                        world.pulling.int2ObjectEntrySet()
                            ...
                            .toList())).apply(instance, CartWorld::new));
    public static final SavedDataType<@NotNull CartWorld> TYPE = new SavedDataType<>(
            HorseCarts.resLoc("world"),
            CartWorld::new,
            CODEC,
            DataFixTypes.LEVEL
    );

    private static CartWorld clientInstance = null;

    private final Int2ObjectMap<UUID> pulling = new Int2ObjectOpenHashMap<>();

    public CartWorld() {}

    private CartWorld(List<EntityWithId> entityWithIds) {
    ...
    public static CartWorld get(Level level) {
        if (level.isClientSide()) {
            return getClient();
        } else {
            return getServer(HorseCarts.server, level.dimension());
        }
    }

    public static CartWorld getClient() {
        if (clientInstance == null) {
            clientInstance = new CartWorld();
        }
        return clientInstance;
    }

    public static CartWorld getServer(MinecraftServer server, ResourceKey<@NotNull Level> levelType) {
