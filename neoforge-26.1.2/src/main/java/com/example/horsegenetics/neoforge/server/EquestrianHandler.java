...
/**
 * Names an <b>equestrian</b> when a villager takes one of this mod's four jobs:
 ...
 * {@link #familySurname}. It does not matter whether that is the cowboy, the
 * leatherworker, the scientist, the supplier or the metalsmith, which is the
 * point: five posts hand out their jobs independently and in whatever order the
 * villagers get round to them, so none of them can be the one that decides.
 *
 * <p>That is why every profession check in this class asks
 * {@link ModVillagerProfessions#isEquestrian} rather than naming one key. A
 * fifth equestrian added later joins the family by existing, and cannot be left
 * out of it by someone forgetting this file.
 *
 ...
 *
 * <p>That also makes it the cheapest answer to the hardest thing about these
 * four to test. They do not generate ready-made - they exist when a post is
...
public final class EquestrianHandler {
    ...
    static void tick(EntityTickEvent.Post event) {
        ...
        if (!ModVillagerProfessions.isEquestrian(villager.getVillagerData().profession())) {
            return;
        }
        ...
        DebugAnnounce.sayAt(level, "Equestrian",
                villager.getName().getString() + " took a post, of the " + surname + " family",
     ...
     * and the four posts hand out their jobs independently, in whatever order the
     * villagers get round to claiming them - so no role can be the one that
     ...
     * post put up beside it joins the Adamses rather than the Hargreaves at the
     * far end.
     *
     * <p>It counts the roles this mod names and nothing else. A player who
     ...
     *
     * <p>An equestrian has no breed of his own to read a region off, so he takes
     * it from the nearest cowboy who has one - the same nearest-wins scan the
     * surname uses, and for the same reason: two outfits in one town are two
     * families from two places, and a post beside the second one joins the second
     * one.
     *
     ...
     * read a region off. It is deliberately <i>not</i> stored on the villager -
