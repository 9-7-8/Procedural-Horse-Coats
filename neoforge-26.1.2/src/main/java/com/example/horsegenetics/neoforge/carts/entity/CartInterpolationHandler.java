...
import com.example.horsegenetics.neoforge.mixin.CartEntityInvoker;
    ...
    public void interpolate() {
        if (!this.hasActiveInterpolation()) {
            ...
            if (this.previousTickPosition != null) {
                ...
                if (this.entity.level().noCollision(this.entity, ((CartEntityInvoker) this.entity).horsegenetics$makeBoundingBox().move(this.interpolationData.position.add(vec3)))) {
            ...
            double dy = Mth.lerp(delta, this.entity.getDeltaMovement().y, this.interpolationData.deltaMovement.y);
            double dz = Mth.lerp(delta, this.entity.getDeltaMovement().z, this.interpolationData.deltaMovement.z);
            Vec3 interpolatedVelocity = new Vec3(dx, dy, dz);
            this.entity.setPos(interpolatedPos);
            ((CartEntityInvoker) this.entity).horsegenetics$setRot(yRot, xRot);
