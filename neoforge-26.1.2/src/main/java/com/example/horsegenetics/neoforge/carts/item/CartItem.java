 ...
 */
package com.example.horsegenetics.neoforge.carts.item;
...
public class CartItem extends Item {
    ...
    private final CartWood woodType;
    private final CartKind cartType;

    public CartItem(CartWood woodType, CartKind cartType, Properties settings) {
        super(settings);
    ...
    public @NotNull InteractionResult use(@NotNull Level level, Player player, @NotNull InteractionHand interactionHand) {
        ...
        if (result.getType() == HitResult.Type.MISS) {
            ...
            if (result.getType() == HitResult.Type.BLOCK) {
                ResourceKey<@NotNull EntityType<?>> key = ResourceKey.create(Registries.ENTITY_TYPE, HorseCarts.resLoc(cartType.id()));
                ...
                if (!level.noCollision(cart, cart.getBoundingBox().inflate(0.1F, -0.1F, 0.1F))) {
                    ...
                    if (!level.isClientSide()) {
                        ...
                        level.playSound(null, cart.getX(), cart.getY(), cart.getZ(), HorseCarts.PLACE_SOUND, SoundSource.BLOCKS, 0.75F, 0.8F);
                        // Where upstream fired its "place a cart" advancement.
                        com.example.horsegenetics.neoforge.server.HorseProgress.complete(
                                player, com.example.horsegenetics.common.progress.ProgressTask.PLACE_CART);
                    }
    ...
    public CartKind getCartType() {
        return cartType;
    }

    /** The wood this cart item builds a cart out of. */
