...
/**
 * Dev-only: <b>right-click an equestrian with a clock to promote them one tier.</b>
 *
 * <h2>Why</h2>
 * The equestrian's interesting trades are at tiers 4 and 5 - the random gene
 * carrots and the research papers - and the honest route to them is several
 * hundred emeralds of trading, per equestrian, per test world. That is not a test,
 ...
 * be. So raising the level without it leaves a Master equestrian still offering
 ...
 */
@EventBusSubscriber
public final class DebugEquestrianLevelUp {

    private DebugEquestrianLevelUp() {
    }
    ...
    static void onInteract(PlayerInteractEvent.EntityInteract event) {
        ...
        if (!VillagerData.canLevelUp(current)) {
            DebugAnnounce.say(level, "Equestrian",
