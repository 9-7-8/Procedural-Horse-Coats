...
| **any file in `horsegenetics/genes/`, or any gene page's `<h1>` or tabs** | `:common:bakeGeneBundle`, `:common:bakeGeneIcons`, `:common:bakeGeneWikiPages`, then `node wiki/tools/bake-gene-timeline.mjs` | `wiki/horse-designer/assets/genes.json`, `wiki/assets/gene-icons/`, the gene's `wiki/gene-*.html`, `wiki/timeline-of-genes.html` **and the generated spans of `wiki/pages.js` and `index.html`** |
