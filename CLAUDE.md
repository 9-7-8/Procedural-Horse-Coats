...
9. **Verification is written on the page for the thing, never on `wiki/verification.html`.**
   (Owner, and it is absolute.) A checklist goes on a **Verification tab of that thing's own
   page** - `data-tab="verification"` per `wiki/tabs.js` - opening with **one sentence in a
   `note verify-summary` box**. `wiki/verification.html` exists to do **nothing except
   collect the pages that have such a tab**: it is generated by
   `bake-verification-index.mjs` and **nothing is ever hand-written onto it**.
   **When something is verified it MOVES: off the Verification tab, onto the Coding tab** of
   the same page, as what was seen, when, and on what evidence. Delete it from
...
| any breed, or `BreedSpecWriter` | `:common:bakeBreedFiles`, then `:common:bakeGeneCensus` - the census's breed column is read off the breed sheets, and nothing guards it the way `BreedFilesTest` guards the files | `common/.../horsegenetics/breeds/`, `wiki/horse-designer/assets/breeds.json` **and** `wiki/gene-census.html` |
